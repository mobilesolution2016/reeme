const char SQLCreateTable_Schedule[] = {
	...
	"	mode INTEGER,"													// 执行模式，可用的值定义在枚举ScheduleMode中
	"	flags INTEGER DEFAULT 0,"										// 一些配置的组合标志
	"	status INTEGER DEFAULT 0,"										// 当前的状态，可用的值定义在ScheduleStatus中
...
const char SQLCreateTable_ScriptTasks[] = {
