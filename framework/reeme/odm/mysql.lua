...
local function read_result(self, max_rows)
	...
	local rowsCount = max_nrows == nil and 0x0FFFFFFF or max_nrows
    local rows = new_tab(max_nrows or 4, 0)
    local i = 0
    while true and i < rowsCount do
        --print("reading a row")
...
local function continue_read_result(self, max_rows)
    if self.state ~= STATE_COMMAND_SENT then
        return nil, "cannot read result in the current context: " .. self.state
    end
	
     local compact = self.compact
	local rowsCount = max_nrows or 0x0FFFFFFF
    local rows = new_tab(max_nrows or 4, 0)
    local i = 0
    while true and i < rowsCount do
        --print("reading a row")
...
_M.continue_read = continue_read_result

local function skip_read_result(self)
    if self.state ~= STATE_COMMAND_SENT then
        return nil, "cannot read result in the current context: " .. self.state
    end
...
_M.skip_result = skip_read_result


function _M.query(self, query, max_rows)
    local bytes, err = send_query(self, query)
    if not bytes then
        return nil, "failed to send query: " .. err
    end

    return read_result(self, max_rows)
end
