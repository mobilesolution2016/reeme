...
local glue = require'reeme.cairo.glue'
local box2d = require'reeme.cairo.box2d'
...
--16bpp RGB565
local function rr(s,i) return      bit.rshift(s[i], 11)      * (255 / 31) end
local function rg(s,i) return bit.band(bit.rshift(s[i],  5), 63) * (255 / 63) end
local function rb(s,i) return bit.band(    s[i],      31) * (255 / 31) end
local function wr(d,i,v) d[i] = bit.bor(bit.band(d[i], 0x07ff), bit.lshift(bit.rshift(r, 3), 11)) end
local function wg(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xf81f), bit.lshift(bit.rshift(g, 2),  5)) end
local function wb(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xffe0), bit.rshift(b, 3)) end
...
local function rr(s,i) return      bit.rshift(s[i], 12)      * (255 / 15) end
local function rg(s,i) return bit.band(bit.rshift(s[i],  8), 15) * (255 / 15) end
local function rb(s,i) return bit.band(bit.rshift(s[i],  4), 15) * (255 / 15) end
local function ra(s,i) return bit.band(    s[i],      15) * (255 / 15) end
local function wr(d,i,v) d[i] = bit.bor(bit.band(d[i], 0x0fff), bit.lshift(bit.rshift(r, 4), 12)) end
local function wg(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xf0ff), bit.lshift(bit.rshift(g, 4),  8)) end
local function wb(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xff0f), bit.lshift(bit.rshift(b, 4),  4)) end
local function wa(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xfff0),     bit.rshift(a, 4)     ) end
local function rrgba(s,i)
	return rr(s,i), rg(s,i), rb(s,i), 0xff
end
local function wrgba(d,i,r,g,b)
...
local function rr(s,i) return      bit.rshift(s[i], 11)      * (255 / 31) end
local function rg(s,i) return bit.band(bit.rshift(s[i],  6), 31) * (255 / 31) end
local function rb(s,i) return bit.band(bit.rshift(s[i],  1), 31) * (255 / 31) end
local function ra(s,i) return bit.band(    s[i],  1)      *  255       end
local function wr(d,i,v) d[i] = bit.bor(bit.band(d[i], 0x07ff), bit.lshift(bit.rshift(r, 3), 11)) end
local function wg(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xf83f), bit.lshift(bit.rshift(g, 3),  6)) end
local function wb(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xffc1), bit.lshift(bit.rshift(b, 3),  1)) end
local function wa(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xfffe),     bit.rshift(a, 7)     ) end
local function rrgba(s,i)
	return rr(s,i), rg(s,i), rb(s,i), 0xff
end
...
local function rr(s,i) return bit.band(bit.rshift(s[i], 10), 31) * (255 / 31) end
local function rg(s,i) return bit.band(bit.rshift(s[i],  5), 31) * (255 / 31) end
local function rb(s,i) return bit.band(    s[i],      31) * (255 / 31) end
local function ra(s,i) return      bit.rshift(s[i], 15)      *  255       end
local function wr(d,i,v) d[i] = bit.bor(bit.band(d[i], 0x83ff), bit.lshift(bit.rshift(r, 3), 10)) end
local function wg(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xfc1f), bit.lshift(bit.rshift(g, 3),  5)) end
local function wb(d,i,v) d[i] = bit.bor(bit.band(d[i], 0xffe0),     bit.rshift(b, 3)     ) end
local function wa(d,i,v) d[i] = bit.bor(bit.band(d[i], 0x7fff), bit.lshift(bit.rshift(a, 7), 15)) end
local function rrgba(s,i)
	return rr(s,i), rg(s,i), rb(s,i), 0xff
end
function wrgba(d,i,r,g,b,a)
	d[i] = bit.bor(bit.lshift(bit.rshift(r, 3), 10),
...
local function paint(src, dst, dstx, dsty, convert_pixel, src_colortype, dst_colortype)
	...
	if src_format == dst_format
		and not convert_pixel
		and src_stride == math.floor(src_stride) --can't copy from fractional offsets
		and dst_stride == math.floor(dst_stride) --can't copy from fractional offsets
		and src_rowsize == math.floor(src_rowsize) --can't copy fractional row sizes
	then
